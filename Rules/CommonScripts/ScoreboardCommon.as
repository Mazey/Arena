...
void drawServerInfo()
{
	...
	string info_gamemode = "Welcome to: " + rules.gamemode_name;
	string info_desc = rules.gamemode_info;
	string info_feedback = "Made by Mazey. Feedback? @mazey01 on Discord";
	SColor white(0xffffffff);
	string mapName = getTranslatedString("Map name : TODO")+rules.get_string("map_name");
	Vec2f dim;
	GUI::GetTextDimensions(info_gamemode, dim);
	if(dim.x + 15 > width)
		width = dim.x + 15;

	GUI::GetTextDimensions(info_desc, dim);
	if(dim.x + 15 > width)
		width = dim.x + 15;

	GUI::GetTextDimensions(info_feedback, dim);
	if(dim.x + 15 > width)
		width = dim.x + 15;
	...
	Vec2f pos(getScreenWidth() - 8 - width / 2, 8);
	...
	Vec2f mid(getScreenWidth() - 8 - width / 2, 8);
	...
	mid.y += 15;
	GUI::DrawTextCentered(info_gamemode, mid, white);
	mid.y += 15;
	GUI::DrawTextCentered(info_desc, mid, white);
	mid.y += 17;
	GUI::DrawTextCentered(info_feedback, mid, white);
	mid.y += 17;
